...
comparisonExps = memoize 'comparisonExps \columnType -> do
  ...
  let name = applyTypeNameCaseCust tCase $ P.getName typedParser <> nameSuffix
      ...
      -- Parser for the inner comparison object used by _any/_all on text arrays.
      -- Returns the first comparison operator found, or Nothing for empty input.
      textArrayElemParser =
        P.object
          [G.name|text_array_comparison_exp|]
          (Just "Text comparison expression for individual array element matching")
          $ fmap (listToMaybe . catMaybes)
          $ sequenceA
            [ P.fieldOptional Name.__eq Nothing ((TCOEq,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__like Nothing ((TCOLike,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__ilike Nothing ((TCOILike,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__nlike Nothing ((TCONLike,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__nilike Nothing ((TCONILike,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__similar Nothing ((TCOSimilar,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__nsimilar Nothing ((TCONSimilar,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__regex Nothing ((TCORegex,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__iregex Nothing ((TCOIRegex,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__nregex Nothing ((TCONRegex,) . IR.mkParameter <$> textParser)
            , P.fieldOptional Name.__niregex Nothing ((TCONIRegex,) . IR.mkParameter <$> textParser)
            ]
  -- Naming conventions
  pure
      ...
      [ flip (maybe []) maybeCastParser $ \castParser ->
        ...
        guard (isScalarColumnWhere (\case PGArray t -> isStringType t; _ -> False) columnType)
          *> [ mkBoolOperator
                 tCase
                 collapseIfNull
                 (C.fromAutogeneratedName Name.__any)
                 (Just "any element of the text array matches the given condition")
                 ( textArrayElemParser
                     `P.bind` \case
                       Just (op, v) -> pure $ ABackendSpecific $ ATextArrayAny op v
                       Nothing -> P.parseError "\"_any\" requires exactly one comparison operator"
                 ),
               mkBoolOperator
                 tCase
                 collapseIfNull
                 (C.fromAutogeneratedName Name.__all)
                 (Just "all elements of the text array match the given condition")
                 ( textArrayElemParser
                     `P.bind` \case
                       Just (op, v) -> pure $ ABackendSpecific $ ATextArrayAll op v
                       Nothing -> P.parseError "\"_all\" requires exactly one comparison operator"
                 )
             ],
        -- Ops for JSONB type
        guard (isScalarColumnWhere (== PGJSONB) columnType)
          *> [ mkBoolOperator
               ...
               mkBoolOperator
                 tCase
                 collapseIfNull
                 (C.fromAutogeneratedTuple $$(G.litGQLIdentifier ["_jsonb", "path", "exists"]))
                 (Just "does the jsonpath return any item for the specified JSON value")
                 (ABackendSpecific . AJsonbPathExists . IR.mkParameter <$> textParser),
               mkBoolOperator
                 tCase
                 collapseIfNull
                 (C.fromAutogeneratedTuple $$(G.litGQLIdentifier ["_jsonb", "path", "match"]))
                 (Just "does the jsonpath predicate check return true for the specified JSON value")
                 (ABackendSpecific . AJsonbPathMatch . IR.mkParameter <$> textParser)
             ],
